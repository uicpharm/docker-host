...
container management according to UIC Pharmacy standards:
   ...
   - ${bold}publish$norm: Takes a Dockerfile and helps publish multi-arch images.
   - ${bold}podman-install-service$norm: On RHEL systems, installs a Podman pod as a service.
...
for cmd in curl cut find install mktemp rsync sort; do
