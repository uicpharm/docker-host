...
[[ -n $(which podman-install-service 2>/dev/null) ]] && HAS_SERVICE_INSTALLER=true || HAS_SERVICE_INSTALLER=false
...
(
   cd "$dir" && \
   docker-compose "${podman_args[@]}" -f "$stack_path" up "${docker_args[@]}" && \
